...
Ao invés de utilizar a função `Marshal` é possível usar a função `Decoder` para transformar um *JSON* em *struc*. A diferença entre essas funções é a forma como se usa cada uma. Na função `Marshal` o resultado é salvo em uma variável que pode ser manipula como o usuário desejar, já na `Decoder` o resultado estará vinculado a uma interface que sempre será chamada quando usarmos a variável na qual está as informações obtidas do *JSON*.

**Exemplo:**
[Exemplo]()

## Transformando JSON para *struc* em Go
De maneira similar a transformar uma *struc* em JSON, também é possível realizar o processo inverso, ou seja, capturar um *JSON* e transportar as informações para uma *struc* em Go utilizando o pacote `enconding/json` e a função `Unmarshal` ou `Decoder`.

**Exemplo:**
[Exemplo](./exemplos/cap16/exemplo_cap16_de_JSON_para_struc.go)

Vale lembrar que no exemplo acima utilizamos *tags*
